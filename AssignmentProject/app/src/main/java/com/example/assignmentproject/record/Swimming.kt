...
data class SwimmingRecord(val date: String, val duration: Float, val distance: Float)

private val SwimmingRecords = listOf(
    SwimmingRecord("2023-04-28", 1.5f, 10.0f),
    SwimmingRecord("2023-04-29", 0.5f, 3.0f),
    SwimmingRecord("2023-04-30", 2.5f, 15.0f),
    SwimmingRecord("2023-05-01", 3.5f, 20.0f),
    SwimmingRecord("2023-05-02", 4.5f, 25.0f),
    SwimmingRecord("2023-05-03", 5.5f, 30.0f),
    SwimmingRecord("2023-05-04", 6.5f, 35.0f),
)


@Preview
@Composable
private fun PreviewSwimmingScreen() {
    SwimmingScreen()
}

@Composable
fun SwimmingScreen(modifier: Modifier = Modifier) {

    CommonListSubTitleScreen(
        title = "Swimming",
        modifier = modifier,
        records = SwimmingRecords,
        itemLayout = {
            val context = LocalContext.current
            val launcher = rememberForResultLauncher()
            Box(modifier = Modifier.clickable {
                launcher.launch(RecordDetailsActivity.action(context, IRecord.Swimming, it))
            }) {
                SwimmingRecordItem(it)
            }
        }
    )
}
...
@Composable
private fun SwimmingRecordItem(record: SwimmingRecord) {
    Row(
        modifier = Modifier
...
// 游泳记录详情
@Composable
fun SwimmingDetailsScreen(record: SwimmingRecord) {
    ...
    Column {
        ScreenTitleText(title = "Swimming Details")
        DetailsItem(
            title = "Date:",
